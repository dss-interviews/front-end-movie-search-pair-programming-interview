...
export const handler: Handlers<SearchData> = {
  async GET(req, ctx) {
    ...
    // Only fetch movies if there are search parameters
    if (query || year) {
      try {
    ...
    return ctx.render({
      ...
      query,
      year,
      sortBy: sortBy as "popularity" | "rating" | "title" | "date",
      loading: false,
      error,
    });
  },
...
export default function Home({ data }: PageProps<SearchData>) {
  const { movies, query, year, sortBy, error } = data;
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8">
                ...
                {movies.map((movie) => (
                  <div key={movie.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="relative">
                      <img
                        src={movie.poster_path || "https://via.placeholder.com/300x450/e5e7eb/6b7280?text=No+Poster"}
                        alt={movie.title}
          ...
          {/* No Results */}
          {(query || year) && movies.length === 0 && !error && (
          ...
          {!query && !year && (
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              ...
              <p className="text-gray-600">Use the search form above to find movies by title or year.</p>
            </div>
          )}
        </div>
      </div>
